...
const CreditPurchaseModal: React.FC<CreditPurchaseModalProps> = ({
  ...
  const retryTokenPurchase = async () => {
    ...
    try {
      console.log(`🔄 Retrying token purchase (attempt ${retryCount + 1})`);
      const res = await postTokenPurchase({ tokenPackageId: selectedPackage });
      ...
      console.error("❌ Token purchase retry failed:", err);
      const errorMessage =
  ...
  const startPollingOrderStatus = (orderId: string) => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        const response = await pollingOrderStatus(orderId);
  ...
  useEffect(() => {
    if (currentStep === 2 && selectedPackage && !qrData && isAuthenticated) {
      ...
      console.log(
        "🔍 Debug: Starting token purchase with package:",
        selectedPackage
      );
      console.log("🔍 Debug: Valid packages are: basic, standard, premium");

      postTokenPurchase({ tokenPackageId: selectedPackage })
        .then((res) => {
          if (!res.data) {
            throw new Error("Không nhận được dữ liệu thanh toán từ server");
          }

          console.log("✅ Token purchase successful:", res.data);
          setQrData(res.data);
          setError(null);
        ...
        .catch((err) => {
          ...
          if (err instanceof Error) {
            if (err.message.includes("TOKEN_PACKAGE_NOT_FOUND")) {
              errorMessage = `Gói token không tồn tại: "${selectedPackage}". Vui lòng chọn gói khác hoặc liên hệ hỗ trợ.`;
            } else if (err.message.includes("Invalid package ID")) {
              errorMessage = `ID gói không hợp lệ: "${selectedPackage}". Các gói hợp lệ: basic, standard, premium`;
            ...
            <Step>
              ...
              <div className="h-full flex flex-col mt-20">
                <div className="space-y-3 flex-1 ">
                  {packages.map((pkg) => {
                    const isStandardPackage = pkg.id === "standard";
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-[#4374FF] text-sm">
                              {pkg.credits}
                            </p>
                          </div>
                        ...
                        <div className="flex-1 space-y-2 text-sm">
                          <div className="flex flex-col items-start">
                            <div className="pt-3 overflow-hidden">
                              <h2 className="text-[#4374FF] text-[16px] font-medium">
                            ...
                            <span className="text-white font-medium text-[14px] uppercase">
                              {qrData?.userBankName || "YOUR TEST ACCOUNT NAME"}
                            </span>
                          </div>
                          ...
                          <div className="flex flex-col items-start">
                            ...
                            <span className="text-white font-medium text-sm font-mono">
                              {qrData?.bankAccount || "YOUR_TEST_ACCOUNT_NUMBER"}
                            </span>
                          </div>
                          ...
                          <div className="flex flex-col items-start">
                            ...
                            <span className="text-white font-semibold text-sm">
                              {qrData?.amount
                                ? `${StringUtils.formatCurrency(qrData.amount)} đ`
                                : `${StringUtils.formatCurrency(
                                    getSelectedPackage()?.numericPrice || 0
                                  )} đ`}
                            </span>
                          </div>
                        </div>
              ...
              <div className="h-full flex flex-col justify-center">
                {isPaymentProcessing ? (
                  ...
                  <div className="text-center space-y-4 mt-20">
                    ...
                    <p className="text-white/70 text-sm">
                      Bạn đã nạp thành công {getSelectedPackage()?.credits} vào
                      tài khoản.
                    </p>
                    <p className="text-white/60 text-sm">
                      Credits sẽ được cập nhật trong vài phút.
                    </p>
