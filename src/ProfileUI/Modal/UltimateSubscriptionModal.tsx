...
import {
  ...
  UltimateSubscriptionQrData,
  UltimateSubscriptionPackage,
} from "../../services/ultimateSubscriptionPurchase";
import QRCode from "qrcode";
import Loading from "../../components/shared/Loading";
import StringUtils from "../../utils/StringUtils";
import googleAuthService from "../../services/GoogleAuthService";

interface UltimateSubscriptionModalProps {
  isOpen: boolean;
  onClose: () => void;
}

const UltimateSubscriptionModal: React.FC<UltimateSubscriptionModalProps> = ({
  isOpen,
  onClose,
}) => {
  ...
  const [qrData, setQrData] = useState<UltimateSubscriptionQrData | null>(null);
  ...
  const packages = getUltimateSubscriptionPackages();
  ...
  const getSelectedPackage = (): UltimateSubscriptionPackage | undefined => {
    return packages.find((pkg) => pkg.id === selectedPackage);
  };

  const retryUltimateSubscriptionPurchase = async () => {
    ...
    try {
      console.log(
        `🔄 Retrying ultimate subscription purchase (attempt ${retryCount + 1})`
      );
      const res = await postUltimateSubscriptionPurchase({
        ultimateSubscriptionPackageId: selectedPackage,
      });
      ...
      console.error("❌ Ultimate subscription purchase retry failed:", err);
      const errorMessage =
  ...
  const startPollingOrderStatus = (orderId: string) => {
    console.log(
      "🔍 Starting polling for ultimate subscription order:",
      orderId
    );
    ...
    const interval = setInterval(async () => {
      try {
        console.log("🔄 Polling ultimate subscription order status...");
        const response = await pollingUltimateSubscriptionOrderStatus(orderId);
        ...
        console.error(
          "❌ Polling ultimate subscription order status failed:",
          err
        );
  ...
  useEffect(() => {
    if (currentStep === 2 && selectedPackage && !qrData && isAuthenticated) {
      ...
      console.log(
        "🔍 Debug: Starting ultimate subscription purchase with package:",
        selectedPackage
      );
      ...
      postUltimateSubscriptionPurchase({
        ...
        .then((res) => {
          ...
          console.log(
            "✅ Ultimate subscription purchase successful:",
            res.data
          );
          setQrData(res);
          setError(null);
        ...
        .catch((err) => {
          console.error("❌ Ultimate subscription purchase failed:", err);
          ...
          if (err instanceof Error) {
            if (
              err.message.includes("ULTIMATE_SUBSCRIPTION_PACKAGE_NOT_FOUND")
            ) {
        ...
        <div className="px-6 pt-2 pb-3 ">
          <Stepper
            ...
            onFinalStepCompleted={() => {
              console.log("Ultimate subscription payment completed!");
            }}
                      ...
                      {error === "Bạn cần đăng nhập để thực hiện thanh toán" ? (
                        <>
                          <button
                            ...
                            className="px-3 py-1.5 bg-[#4285F4] hover:bg-[#4285F4]/80 text-white rounded-lg transition-colors flex items-center space-x-2 text-sm cursor-pointer"
                          >
                        ...
                        Thông tin thanh toán
                          ...
                          <div className="flex flex-col items-start">
                            ...
                            <span className="text-white font-semibold text-sm">
                              {getSelectedPackage()?.price || "$24.9 USD"}
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      {countdownActive && (
                        <div className="flex items-start gap-2 py-2 relative bottom-[30px]">
                          <img 
                            ...
                            alt="Timer" 
                            className={`w-5 h-5 text-white ${countdownActive ? 'animate-spin-slow' : ''}`}
                          />
              ...
              <div className="h-full flex flex-col justify-center">
                {isPaymentProcessing ? (
                  ...
                  <div className="text-center space-y-4 mt-12">
                    ...
                    <p className="text-white/70 text-sm">
                      Bạn đã đăng ký thành công gói ULTIMATE{" "}
...
export default UltimateSubscriptionModal;
