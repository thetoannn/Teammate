    ...
    if (enabled) {
      ...
      if (selectedToolsList.length > 0) {
        ...
        localStorage.setItem(
          ...
          JSON.stringify(
            allTools
              .filter((t) => !selectedToolsList.includes(t))
              .map((t) => t.id)
          )
        );
      }
    ...
    >
      ...
      >
        ...
        <Space size="small">
          ...
          <Switch checked={autoMode} onChange={handleAutoToggle} size="small" />
        </Space>
      </div>
      ...
      >
        {Object.entries(getCurrentModels()).map(
          ([provider, providerModels], index, array) => {
            ...
            return (
              <div key={provider}>
                ...
                {providerModels.map((model: ModelInfo | ToolInfo) => {
                  ...
                  const modelName =
                    activeTab === "text"
                      ...
                      : (model as ToolInfo).display_name ||
                        (model as ToolInfo).id;
                  return (
                    <div
                      key={modelKey}
                      style={{
                        ...
                        backgroundColor: isModelSelected(modelKey)
                          ? "#4374FF"
                          : "transparent",
                        border: isModelSelected(modelKey)
                          ? "1px solid #4374FF"
                          : "1px solid transparent",
                      }}
                      onMouseEnter={(e) => {
                        e.currentTarget.style.backgroundColor = "#4374FF";
                      }}
                      onMouseLeave={(e) => {
                ...
                {!isLastGroup && (
                  <Divider style={{ margin: "12px 0", borderColor: "#555" }} />
                )}
              </div>
            );
          }
